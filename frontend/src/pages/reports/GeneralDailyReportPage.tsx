...
function previousDayISO(isoDay: string): string {
...
export function GeneralDailyReportPage() {
  ...
  const previousTo = from ? previousDayISO(from) : ''
  const { data: previousCashOutData } = useGetCashOutDailyReportQuery(
    {
      businessId: effectiveBusinessId,
      to: !dateRangeInvalid && from ? `${previousTo}T23:59:59` : undefined,
      stationId: apiStationId,
    },
    { skip: effectiveBusinessId <= 0 || dateRangeInvalid || needsWorkspaceStation || !from },
  )
  ...
  const previousSalesGrand = useMemo(() => {
    let subtotalLocal = 0
    let usdOnly = 0
    let sspToUsd = 0
    if (!from) return { subtotalLocal, usdOnly, sspToUsd, totalDollar: 0 }
    for (const inv of inventoriesData?.items ?? []) {
      const day = inv.date.slice(0, 10)
      ...
      const usdAmount = Number(inv.usdAmount) || 0
      const rate = Number(inv.exchangeRate) || 0
      subtotalLocal += sspAmount
      usdOnly += usdAmount
      sspToUsd += rate > 0 ? sspAmount / rate : 0
    }
    return { subtotalLocal, usdOnly, sspToUsd, totalDollar: usdOnly + sspToUsd }
  }, [inventoriesData?.items, from, nozzleById, dippingFuelTypeById, fuelNameById])
  ...
  const cashOutTotals = useMemo(() => {
    ...
    for (const r of lines) {
      ...
      if (code === 'USD') {
        usdCurrencyOnly += Number(r.localAmount) || 0
      } else {
        localNonUsd += Number(r.localAmount) || 0
      }
    }
    return { local: localNonUsd, usd: usdCurrencyOnly }
  }, [cashOutData?.lines])
  const previousCashOutTotals = useMemo(() => {
    const lines = previousCashOutData?.lines ?? []
    let localNonUsd = 0
    let usdCurrencyOnly = 0
    for (const r of lines) {
      const code = (r.currencyCode || '').trim().toUpperCase()
    ...
    return { local: localNonUsd, usd: usdCurrencyOnly }
  }, [previousCashOutData?.lines])

  const dailySummary = useMemo(() => {
    const salesLocal = dailyCashGrand.subtotalLocal
    const salesSspToUsd = dailyCashGrand.sspToUsd
    const salesUsd = dailyCashGrand.totalDollar
    const periodFinalUsd = dailyCashGrand.finalUsd
    const previousBalanceLocal = previousSalesGrand.subtotalLocal - previousCashOutTotals.local
    const prevSspSales = previousSalesGrand.subtotalLocal
    const previousBalanceSspToUsd =
      prevSspSales > 1e-9
        ? previousSalesGrand.sspToUsd * (Math.max(0, previousBalanceLocal) / prevSspSales)
        : 0
    const previousBalanceUsd = previousSalesGrand.usdOnly - previousCashOutTotals.usd
    const { local: outLocal, usd: outUsd } = cashOutTotals
    const totalLocal = salesLocal + previousBalanceLocal
    const totalSspToUsd = salesSspToUsd + previousBalanceSspToUsd
    const totalUsd = salesUsd + previousBalanceUsd
    const finalBalanceUsd = totalUsd + totalSspToUsd - outUsd
    return {
      salesLocal,
      salesSspToUsd,
      salesUsd,
      periodFinalUsd,
      previousBalanceLocal,
      previousBalanceSspToUsd,
      previousBalanceUsd,
      totalLocal,
      totalSspToUsd,
      totalUsd,
      outLocal,
      outUsd,
      balanceLocal: totalLocal - outLocal,
      balanceUsd: totalUsd - outUsd,
      finalBalanceUsd,
    }
  }, [dailyCashGrand, previousSalesGrand, previousCashOutTotals, cashOutTotals])
  ...
  function handleOpenPdf() {
    ...
    if (
      ...
      if (cashOutLines.length === 0) {
        ...
        for (const r of cashOutLines) {
          ...
          summaryBody.push([
            ...
            isUsd ? '$0.00' : `$${formatDecimal(r.amountUsd)}`,
            '',
            '',
          ])
        }
      ...
      summaryBody.push([
        ...
        { content: 'Total Dollar Out', colSpan: 2, styles: { halign: 'left' } },
          ...
          <table className="min-w-full border-collapse text-sm">
            <tbody>
              ...
              <tr className="bg-white">
                ...
                <th className="border border-slate-300 px-4 py-2 text-center text-base font-medium text-slate-900">USD</th>
              ...
              <tr className="font-medium">
                ...
                <td colSpan={2} className="border border-slate-300 px-4 py-2 text-2xl text-slate-900">Total Dollar Out</td>
                <td colSpan={2} className="border border-slate-300 px-4 py-2 text-right text-2xl tabular-nums text-slate-900">
              ...
              {cashOutLines.length > 0 && (
                <tfoot className="border-t border-slate-300 bg-slate-100/80">
                  <tr>
                    ...
                    <td className="px-4 py-2" />
                    <td className="px-4 py-2 text-right font-bold tabular-nums text-slate-900">
                      ${formatDecimal(cashOutTotals.usd)}
                    </td>
                  </tr>
                </tfoot>
              )}
